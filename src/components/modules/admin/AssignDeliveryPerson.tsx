...
export function AssignDeliveryPerson({ children, parcelId }: AssignProps) {
  ...
  // console.log(data)
  const deliveryPersons = data?.data?.users?.filter(
    (user: { role: string }) => user.role == Role.DELIVERY_PERSON
  );
          ...
          <Table className="px-10">
            ...
            <TableBody className="">
              {deliveryPersons?.map((user: IUser) => (
                <TableRow>
                  <TableCell className="border-2 bg-blue-50">
                    {user.name}
                  </TableCell>
                  <TableCell className="border-2 bg-gray-50">
                    {user.email}
                  </TableCell>
                  <TableCell className="border-2 bg-orange-50">
                    {user.phone}
                  </TableCell>
                  <TableCell className="border-2 bg-red-50">
                    {user.isActive}
                  </TableCell>

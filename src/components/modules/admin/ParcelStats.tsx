...
const ParcelStats: React.FC<ParcelStatsProps> = ({ parcelStats }) => {
  const deliveredPercentage =
    parcelStats?.totalParcels > 0
      ? Math.round(
          ((parcelStats?.parcelsByStatus?.find((s) => s._id === "DELIVERED")
            ?.count || 0) /
  ...
  const growthRate =
    parcelStats?.parcelsLast30Days > 0
      ? Math.round(
          ((parcelStats?.parcelsLast7Days * 4) /
            parcelStats?.parcelsLast30Days) *
            100
        )
      : 0;
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge className="bg-blue-100 text-blue-800 text-xs">
                      <TrendingUp size={12} />
                      <span className="ml-1">
                        +{parcelStats?.parcelsLast30Days} this month
                      </span>
                    </Badge>
                  </div>
                </div>
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm text-gray-500">
                      {parcelStats?.totalParcels > 0
                        ? Math.round(
                            (sender.parcelCount / parcelStats?.totalParcels) *
                              100
                          )
                        : 0}
                      % of total
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4">
              {parcelStats?.parcelsByStatus?.map((status) => (
                <div
                  key={status._id}
                  className="text-center p-3 bg-gradient-to-br from-gray-50 to-white dark:from-gray-700 dark:to-gray-600 rounded-lg"
                >
                  ...
                  <div className="text-xs text-gray-500">
                    {parcelStats?.totalParcels > 0
                      ? Math.round(
                          (status.count / parcelStats?.totalParcels) * 100
                        )
                      : 0}
                    %
                  </div>

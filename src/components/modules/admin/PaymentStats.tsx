...
const PaymentStats: React.FC<PaymentStatsProps> = ({ paymentStats }) => {
  const successRate =
    paymentStats?.totalPayment > 0
      ? Math.round(
          ((paymentStats?.totalPaymentByStatus?.find((s) => s._id === "PAID")
            ?.count || 0) /
            paymentStats?.totalPayment) *
            100
        )
      : 0;

  const pendingPayments =
    paymentStats?.totalPaymentByStatus?.find((s) => s._id === "PENDING")
      ?.count || 0;
  const failedPayments =
    paymentStats?.totalPaymentByStatus?.find((s) => s._id === "FAILED")
      ?.count || 0;
  const cancelledPayments =
    paymentStats?.totalPaymentByStatus?.find((s) => s._id === "CANCELLED")
      ?.count || 0;
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 p-6">
      <div className="max-w-7xl mx-auto space-y-8">
              ...
              <div className="flex items-center justify-between">
                <div className="space-y-2">
                  ...
                  <p className="text-3xl font-bold text-green-600">
                    {paymentStats?.totalPaymentByStatus?.find(
                      (s) => s._id === "PAID"
                    )?.count || 0}
                  </p>
                  <div className="flex items-center gap-2">
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm text-gray-500">
                      {paymentStats?.totalPayment > 0
                        ? Math.round(
                            (gateway.count / paymentStats?.totalPayment) * 100
                          )
                        : 0}
                      % of total
                    </span>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {paymentStats?.totalPaymentByStatus?.map((status) => (
                <div
                  key={status._id}
                  className="text-center p-4 bg-gradient-to-br from-gray-50 to-white dark:from-gray-700 dark:to-gray-600 rounded-lg"
                >
                  ...
                  <div className="text-xs text-gray-500">
                    {paymentStats?.totalPayment > 0
                      ? Math.round(
                          (status.count / paymentStats?.totalPayment) * 100
                        )
                      : 0}
                    % of total
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <div className="bg-gradient-to-br from-red-50 to-rose-50 dark:from-red-900/20 dark:to-rose-900/20 p-4 rounded-lg">
                ...
                <div className="text-lg font-semibold text-red-600">
                  {failedPayments + cancelledPayments}
                </div>
        ...
        {(!paymentStats?.totalRevenue ||
          paymentStats?.totalRevenue.length === 0) && (
          <Card className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl border-0 shadow-xl">
            <CardContent className="p-12 text-center">
              ...
              <p className="text-gray-600 dark:text-gray-400">
                Revenue tracking and analytics will be available once payment
                processing is fully configured.
              </p>
            </CardContent>
          </Card>
        )}

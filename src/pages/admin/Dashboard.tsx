...
const ParcelStats = React.lazy(
  () => import("@/components/modules/admin/ParcelStats")
);
const PaymentStats = React.lazy(
  () => import("@/components/modules/admin/PaymentStats")
);
const UserStats = React.lazy(
  () => import("@/components/modules/admin/UserStats")
);
import {
  useGetParcelStatsQuery,
  useGetPaymentStatsQuery,
  useGetUsersStatsQuery,
} from "@/redux/feature/stats/stats.api";

import { Skeleton } from "@/utils/Skeleton";

const Dashboard = () => {
  const { data: userStats, isLoading: userLoading } =
    useGetUsersStatsQuery(undefined);
  const { data: paymentStats, isLoading: paymentLoading } =
    useGetPaymentStatsQuery(undefined);
  const { data: parcelStats, isLoading: parcelLoading } =
